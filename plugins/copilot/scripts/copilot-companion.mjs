...
function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!copilotStatus.available) {
    nextSteps.push("Install the Copilot CLI. See https://docs.github.com/copilot/how-tos/copilot-cli");
  }
...
function ensureCopilotReady(cwd) {
  ...
  if (!authStatus.available) {
    throw new Error("Copilot CLI is not installed. Install it from https://docs.github.com/copilot/how-tos/copilot-cli, then rerun `/copilot:setup`.");
  }
...
async function executeReviewRun(request) {
  ...
  const prompt = reviewName === "Review" && !focusText
    ? `/review`
    : buildAdversarialReviewPrompt(context, focusText);

  const useStructuredOutput = reviewName !== "Review" || focusText;
  const result = await runCopilotTurn(context.repoRoot, {
    prompt,
    model: request.model,
    effort: request.effort ?? (useStructuredOutput ? "high" : "medium"),
